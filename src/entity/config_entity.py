...
class InferenceFeatureMatrixBuilderConfig:
    """
    Configuration for the Input Feature Matrix Builder component.
    
    Responsibilities:
    - Defines local scratchpad paths for the generated Parquet matrix and metadata.
    - Specifies the upstream Data Lake source directory (Bronze/Silver layer).
    - Enforces the Hive-partitioning flag required by the shared SQL module.
    """

    def __init__(self, inference_pipeline_config) -> None:
        """
        Initializes the configuration using the base pipeline run directory.
        
        Args:
            inference_pipeline_config: The root pipeline configuration containing the run_id
                                       and root_dir for the current inference execution.
        """
        try:
            # Component-specific isolated root directory
            self.matrix_builder_root_dir: str = os.path.join(
                inference_pipeline_config.root_dir,
                constants.MATRIX_BUILDER_ROOT_DIR_NAME,
            )

            # Local scratchpad paths for DuckDB outputs
            self.feature_matrix_file_path: str = os.path.join(
                self.matrix_builder_root_dir,
                constants.MATRIX_BUILDER_OUTPUT_FILE_NAME,
            )
            self.metadata_file_path: str = os.path.join(
                self.matrix_builder_root_dir,
                constants.MATRIX_BUILDER_METADATA_FILE_NAME,
            )

            # Upstream Data Lake configurations
            # Defaults to local "company_data/silver" (generated by synthetic_data_generator.py) 
            # but can be overridden via environment variables for AWS S3 execution in production.
            self.company_data_dir: str = os.getenv("COMPANY_DATA_DIR", "company_data/silver")
            
            # Inference always reads from the simulated ETL output, which uses Hive-style partitioning.
            self.is_partitioned: bool = True

            # Pre-create the directory structure to prevent FileNotFoundError during DuckDB streaming
            os.makedirs(self.matrix_builder_root_dir, exist_ok=True)
            logging.info(
                "InferenceFeatureMatrixBuilderConfig initialized. Upstream Data Source: %s | Partitioned: %s",
                self.company_data_dir,
                self.is_partitioned
            )

        except Exception as e:
            logging.exception("Error initializing InferenceFeatureMatrixBuilderConfig.")
            raise CustomException(e, sys) from e